...
export default function ChatPage() {
  ...
  useEffect(() => {
    const loadConversations = async () => {
      // Validamos que exista el ID del usuario antes de llamar a la API
      if (!session?.user?.id) return;

      try {
        setIsLoading(true);

        // CORRECCIÓN 1: Usamos ApiClient.chat.getUserConversations
        // Pasamos el ID del usuario actual
        const result = await ApiClient.chat.getUserConversations(
          session.user.id
        );

        // CORRECCIÓN 2: Accedemos a result.data.conversations
