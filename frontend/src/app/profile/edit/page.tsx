...
export default function EditProfile() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ Enviamos la data. El microservicio de usuarios ya tiene carrera y semestre de la DB
      const result = (await ApiClient.users.updateProfile(
        session!.user.id,
        formData,
      )) as any;

      if (!result.success) throw new Error("Fallo al guardar");

      setSuccess("¡Perfil actualizado con éxito!");

      // Forzamos actualización de sesión y redirección fuerte
      await update();
      setTimeout(() => {
